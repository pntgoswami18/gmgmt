...
const createReferral = async (req, res) => {
  try {
    ...
    const referralEnabled = settingsCache.getBoolean('referral_system_enabled', false);
    ...
    const discountAmount = parseFloat(settingsCache.get('referral_discount_amount', '0')) || 0;

    // Check if referrer exists and is not admin
    const referrerResult = await pool.query('SELECT id, is_admin FROM members WHERE id = $1', [
      referrer_id,
    ]);
    ...
    const existingReferral = await pool.query('SELECT id FROM referrals WHERE referred_id = $1', [
      referred_id,
    ]);
...
const applyReferralDiscount = async (req, res) => {
  try {
    ...
    await pool.query('UPDATE referrals SET status = $1, applied_at = $2 WHERE id = $3', [
      'applied',
