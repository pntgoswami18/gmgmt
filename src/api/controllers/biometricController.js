...
const startEnrollment = async (req, res) => {
  try {
    ...
    const memberResult = await pool.query(
      'SELECT id, name, biometric_id FROM members WHERE id = ?',
      [memberId]
    );
    ...
    res.json({
      ...
      data: {
        ...
        instructions:
          'Please ask the member to place their finger on the biometric device multiple times as instructed.',
      },
    });
  } catch (error) {
...
const getEnrollmentStatus = async (req, res) => {
  try {
    ...
    const isActive =
      biometricIntegration.enrollmentMode && biometricIntegration.enrollmentMode.active;
...
const getBiometricEvents = async (req, res) => {
  try {
    ...
    // Get total count
    let countQuery =
      'SELECT COUNT(*) as total FROM biometric_events be LEFT JOIN members m ON be.member_id = m.id WHERE 1=1';
    let countParams = [];
    ...
    if (biometricIntegration) {
      ...
      try {
        ...
        if (tableCheck.rows.length === 0) {
          logger.warn('⚠️ biometric_events table does not exist');
...
const getMembersWithoutBiometric = async (req, res) => {
  try {
    ...
    logger.info(`Found ${members.length} members without biometric data (page ${pageNum})`);
...
const getMembersWithBiometric = async (req, res) => {
  try {
    ...
    logger.info(`Found ${members.length} members with biometric data (page ${pageNum})`);
...
const manualEnrollment = async (req, res) => {
  try {
    ...
    const existingResult = await pool.query('SELECT id, name FROM members WHERE biometric_id = ?', [
      deviceUserId,
    ]);
    ...
    try {
      ...
      if (whatsappResult.success) {
        logger.info(`📱 WhatsApp welcome message prepared for ${member.name} (manual enrollment)`);
      } else {
        logger.info(
  ...
  try {
    ...
    if (event === 'heartbeat') {
      ...
      if (status === 'enrollment_progress') {
        ...
        try {
          logger.info(`📤 Calling handleEnrollmentData for progress updates`);
        ...
        try {
          ...
          if (
            ...
            logger.info(`🛑 Enrollment mode stopped for member ${canonicalMemberId}`);
          }
    ...
    // Handle restore_success from ESP32 (sent after restore_fingerprint command)
    if (event === 'Restore' && status === 'restore_success') {
      eventType = 'restore';
      memberIdToUse = memberId;
      biometricId = userId; // new sensor slot assigned by ESP32
      success = true;
      try {
...
const updateMemberCache = async (req, res) => {
  try {
    ...
    logger.info(
      `🔄 Cache update request from device: ${deviceId} (page: ${page || 'all'}, pageSize: ${pageSize || 'all'})`
    );
    ...
    logger.info(
      `✅ Sending cache update: ${cacheMembers.length} members to device ${deviceId} (page ${page || 'all'})`
    );
    res.json(response);
  } catch (error) {
...
const invalidateESP32Cache = async () => {
  try {
    logger.info('🔄 Triggering immediate ESP32 cache invalidation...');
    ...
    if (devices.rows.length === 0) {
      logger.info('⚠️ No active ESP32 devices found for cache invalidation');
      return;
    }

    logger.info(`📡 Found ${devices.rows.length} active ESP32 devices for cache invalidation`);
    ...
    for (const device of devices.rows) {
      try {
        ...
        logger.info(`📡 Sending cache invalidation to: ${deviceUrl}`);
...
const syncBiometricData = async (_req, res) => {
  try {
    ...
    logger.info('🔄 Biometric data sync requested');
