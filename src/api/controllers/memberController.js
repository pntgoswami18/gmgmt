...
exports.createMember = async (req, res) => {
  ...
  try {
    ...
    if (!isValidPhone(phone)) {
      return res
        .status(400)
        .json({ message: 'Invalid phone number. Use 10–15 digits, with optional leading +' });
    }
    const planId =
      membership_plan_id === null || membership_plan_id === undefined || membership_plan_id === ''
        ? null
...
exports.updateMember = async (req, res) => {
  const { id } = req.params;
  const { name, phone, address, birthday, photo_url, is_admin, membership_plan_id, join_date } =
    req.body;
  try {
    // Load existing to allow partial updates while ensuring mandatory fields present post-update
    const existingRes = await pool.query(
      'SELECT name, phone, address, birthday, photo_url, is_admin, membership_plan_id FROM members WHERE id = $1',
      [id]
    );
    ...
    if (!finalPhone) {
      return res
        .status(400)
        .json({ message: 'Phone is required and must be 10–15 digits (optional leading +).' });
    }
    ...
    const safeJoinDate =
      join_date === undefined
        ? existing.join_date || new Date().toISOString().split('T')[0]
        : join_date;
  ...
  try {
    ...
    if (device_user_id) {
      ...
      const existingMember = await pool.query(
        'SELECT id, name FROM members WHERE biometric_id = ? AND id != ?',
...
exports.setActiveStatus = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (val === 0 && deleteFingerprint) {
        await deleteFingerprint(id);
      } else if (val === 1 && restoreFingerprint) {
