...
exports.bookClass = async (req, res) => {
  ...
  try {
    ...
    const schedule = await pool.query('SELECT max_capacity FROM class_schedules WHERE id = $1', [
      schedule_id,
    ]);
    ...
    const bookingCount = await pool.query(
      'SELECT COUNT(*) as cnt FROM bookings WHERE schedule_id = $1',
      [schedule_id]
    );
...
exports.cancelBooking = async (req, res) => {
  ...
  try {
    ...
    const cancelledBooking = await pool.query(`SELECT * FROM bookings WHERE id = $1`, [bookingId]);
