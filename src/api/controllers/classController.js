...
const logger = require('../../utils/logger').child({ service: 'classController' });
...
exports.createClass = async (req, res) => {
  ...
  try {
    ...
    const adminCheck = await pool.query(
      'SELECT id, name FROM members WHERE name = $1 AND is_admin = 1',
      [instructor]
    );
    if (adminCheck.rows.length === 0) {
      return res.status(400).json({
        message:
          'Instructor must be an admin member. Only admin users can be assigned as class instructors.',
      });
    }
...
exports.updateClass = async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'UPDATE classes SET name = $1, description = $2, instructor = $3, duration_minutes = $4 WHERE id = $5',
...
exports.getAdminInstructors = async (req, res) => {
  try {
    const adminMembers = await pool.query(
      'SELECT id, name, is_admin FROM members WHERE is_admin = 1 ORDER BY name ASC'
    );
