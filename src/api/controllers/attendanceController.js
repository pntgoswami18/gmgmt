...
const performCheckIn = async (resolvedMemberId, res) => {
  ...
  const member = await pool.query('SELECT id, is_active FROM members WHERE id = $1', [
    resolvedMemberId,
  ]);
  ...
  const isInMorningSession =
    minutesSinceMidnight >= MORNING_START_MINUTES && minutesSinceMidnight <= MORNING_END_MINUTES;
  const isInEveningSession =
    minutesSinceMidnight >= EVENING_START_MINUTES && minutesSinceMidnight <= EVENING_END_MINUTES;

  if (!isInMorningSession && !isInEveningSession) {
    return res.status(400).json({
      message: `Check-in allowed only during Morning (${settingsMap.morning_session_start || '05:00'}-${settingsMap.morning_session_end || '11:00'}) or Evening (${settingsMap.evening_session_start || '16:00'}-${settingsMap.evening_session_end || '22:00'}) sessions.`,
    });
  }
  ...
  const newAttendance = await pool.query(
    'SELECT * FROM attendance WHERE member_id = $1 ORDER BY id DESC LIMIT 1',
    [resolvedMemberId]
  );
