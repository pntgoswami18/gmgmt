...
router.get('/list', async (_req, res) => {
  try {
    ...
    logger.error('Error listing firmware:', error);
...
router.get('/download/:id', async (req, res) => {
  try {
    ...
    if (!resolvedPath.startsWith(path.resolve(FIRMWARE_DIR))) {
      logger.error('Path traversal attempt blocked for firmware id:', id);
...
router.post('/update/:deviceId', requireSameOrigin, async (req, res) => {
  try {
    ...
    try {
      ...
      logger.error('OTA command failed:', cmdError);
      await pool.query(
...
router.get('/log', async (req, res) => {
  try {
    ...
    logger.error('Error fetching update log:', error);
