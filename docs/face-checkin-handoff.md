...
| — | gmgmt | Windows compatibility: port `deploy-models.sh`/`download-models.sh` to Node | ✅ Merged ([gmgmt#37](https://github.com/pntgoswami18/gmgmt/pull/37)) (see §7) |
| — | gmgmt | Windows compatibility: add missing `ESP32_DEPLOYMENT_GUIDE.md` + `tools/WINDOWS_TESTING_GUIDE.md` | ✅ Merged ([gmgmt#38](https://github.com/pntgoswami18/gmgmt/pull/38)) (see §7) |
| — | gmgmt | Automate `deploy-models.js` on server boot when its prerequisites already exist | ✅ Done (see §3.4) |
| — | gmgmt | Publish the embedder as a downloadable, sha256-pinned artifact; run predeploy from `src/app.js` too (installed Windows Service bypasses npm hooks) | ✅ Done this pass (see §3.4) |
...
| — | ops | `npm install` in `client/` on the deployment target | ⬜ Not started (the embedder itself no longer needs a local `convert.py` run — see §3.4) |
| — | ops | Provision kiosk browser with `DEVICE_SHARED_SECRET` | ⬜ Not started (Windows Service installs already have the secret itself — auto-generated, see §3.5. Ops still needs to read it out of `.env` and enter it into the kiosk) |
...
`tools/face-model/deploy-models.js` (originally `.sh`, merged in #21; **ported to cross-platform Node** — see §7) copies the fp32 embedder + pinned `face_landmarker.task` + WASM runtimes into `public/models/` and writes `manifest.json`. **`public/models/` is gitignored** — this must be *run*, not just merged, on every environment that needs to serve the kiosk (dev, staging, prod). `GET /api/biometric/face/model-manifest` 404s otherwise (see `getModelManifest` in `faceBiometricController.js`).

**The embedder is a published, sha256-pinned release asset — no local Python build required to deploy it.** `face_embedder_v1_fp32.tflite` (built once via `convert.py`, cosine fidelity 1.0 vs the ONNX source — see Phase 1 results in `tools/face-model/README.md`) is published as a GitHub Release asset: [`face-model-v1`](https://github.com/pntgoswami18/gmgmt/releases/tag/face-model-v1), sha256 `f2fde3b5a49508c1f1d2ff34fb2c612163ec9d945cc0431efbbc9ebbed16ec7b`. `deploy-models.js` prefers a local `tools/face-model/build/face_embedder_v1_fp32.tflite` if a maintainer is actively iterating on a new revision (from a fresh `convert.py` run), and otherwise fetches the pinned release asset via the same sha256-verified `fetchVerified` helper `download-models.js` already uses for the other pinned artifacts. Publishing a new model revision means re-uploading the release asset and bumping `EMBEDDER_URL`/`EMBEDDER_SHA256` in `deploy-models.js` together, deliberately — same discipline as bumping `ZOO_REF` in `download-models.js`.

This closes a real deployment gap discovered this pass: `public/models/` and `tools/face-model/build/` are both gitignored, so the *only* copy of a locally-built embedder lived on whichever maintainer's machine last ran `convert.py`. Installing GMgmt fresh on a new machine left face check-in broken (`model-manifest` 404) until the file was hand-copied over. Now any machine can produce it from just the release asset — no Python/TensorFlow toolchain needed at deploy time.

**Automatic in two places, covering both real boot paths.** `package.json`'s `prestart`/`predev` invoke `tools/face-model/predeploy-models.js`'s `runPredeploy()` on every `npm start`/`npm run dev`. Separately, `src/app.js` calls the same `runPredeploy()` directly at server boot — this is the fix for the actual production path: the installed Windows Service launches `node.exe src/app.js` straight from `scripts/service-install.js` (see `installer/gmgmt-installer.nsi`), which **never goes through npm at all**, so the `prestart` hook alone never protected it. Both call sites are soft/non-fatal (`deploy-models.js` exports `deploy()`/`checkPrerequisites()` for this) — if `client/node_modules` isn't installed, or the network fetch fails, it logs and skips; it never blocks or fails server boot. `src/app.js` doesn't await it, so a slow/offline first-run download can't delay the server coming up — face check-in just stays unreachable (404 on model-manifest, `modelsReady: false` from `GET /api/biometric/face/config`) until the download finishes in the background. Manual invocation (`node tools/face-model/deploy-models.js`) still works exactly as before and still hard-fails loudly on any problem.

**Why not bake the model into the installer instead?** Considered and rejected for v1: `scripts/build-installer.js` doesn't currently touch `public/models/` at all, so baking it in would mean teaching the installer build to invoke `deploy-models.js` at packaging time (a new packaging-time network dependency, one more thing to go wrong per-arch) just to grow the `.exe` by ~40MB — without even fixing the Windows Service gap above, since that's a boot-time problem, not a packaging one. First-run download needs one clear on-target internet connection instead, surfaced via `modelsReady`.

**Correction (this pass): `DEVICE_SHARED_SECRET` is NOT manual-only — it's auto-generated for the actual production path.** The two paragraphs this replaces claimed it "stays manual by deliberate choice." That was true of the model-deploy automation covered above, but doesn't describe `DEVICE_SHARED_SECRET` itself: `scripts/lib/serviceEnv.js`'s `ensureGeneratedSecrets()` (called from `ensureServiceEnvironment()`, which only `scripts/service-install.js`/`service-manage.js` invoke — i.e. the Windows Service install/management path, not `npm start`/`npm run dev`) generates a real `crypto.randomBytes(32)` value whenever `.env` doesn't already have one, specifically so "the ESP32 device endpoints... never run unauthenticated in production" (its own doc comment). Verified live on a real installed service: a truly fresh install (wiped `ProgramData\gmgmt`, no prior `.env`) came up with `deviceSecretConfigured: true` and a unique secret in `.env`, with no admin action taken. `ENABLE_BIOMETRIC` and `face_checkin_enabled` are still genuinely manual — this correction is scoped to the shared secret only.

### 3.5 `DEVICE_SHARED_SECRET` + provisioning the kiosk browser
- On a Windows Service install, this is already set — `ensureGeneratedSecrets()` (see above) generates it on first boot if `.env` doesn't have one; nothing to do here beyond reading the value back out of `%ProgramData%\gmgmt\.env` to hand to the kiosk. On a non-service boot (dev, `npm start`, manual `node src/app.js`), it's still unset by default (per `.env.sample:29`) and must be set by hand. Until it's set either way, `/api/biometric/face/sync` and `/face/check-in` return 503 (see `src/app.js:133-139`) — deliberate fail-closed behavior, not a bug.
