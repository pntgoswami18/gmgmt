...
| — | gmgmt | Automate `deploy-models.js` on server boot when its prerequisites already exist | ✅ Done this pass (see §3.4) |
...
| — | ops | Build the model once (`convert.py`) + `npm install` in `client/` on the deployment target | ⬜ Not started (`deploy-models.js` itself now runs automatically on `npm start`/`npm run dev` once these exist — see §3.4) |
...
**Now automatic when its prerequisites already exist.** `package.json`'s `prestart`/`predev` invoke `tools/face-model/predeploy-models.js` — a soft, non-fatal wrapper (`deploy-models.js` exports `deploy()`/`checkPrerequisites()` for this) that runs the deploy automatically on every `npm start`/`npm run dev` boot *if* `build/face_embedder_v1_fp32.tflite` (from `convert.py`) and `client/node_modules` (from `npm install` in `client/`) are already present. If either is missing — the common case on a machine that hasn't built the model yet, or any boot where face check-in is irrelevant — it logs one quiet informational line and exits 0; it never blocks or fails a normal server boot. Verified end-to-end: a real `npm start` with the prerequisites present correctly ran the deploy before the server booted (`public/models/manifest.json` populated, server up clean); with `build/` moved aside, `npm start` still booted cleanly with just the skip message. Manual invocation (`node tools/face-model/deploy-models.js`) still works exactly as before and still hard-fails loudly on any problem — this automation only wraps it for the boot-time case, it doesn't replace it.
