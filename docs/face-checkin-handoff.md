...
**Not automated, and won't be:** `DEVICE_SHARED_SECRET`, `ENABLE_BIOMETRIC`, and `face_checkin_enabled` all stay manual by deliberate choice — auto-generating/enabling any of them would let face check-in start accepting scans (and unlocking a physical door) without an admin ever reviewing it. Scope was explicitly narrowed to model deployment only when this automation was built.
