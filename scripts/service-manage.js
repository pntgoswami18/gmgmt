...
// into the service definition and silently override later edits to
// %ProgramData%\gmgmt\.env (dotenv never overrides pre-set variables).
const svc = new Service({
  name: 'GMgmt',
  script: path.join(projectRoot, 'src', 'app.js'),
  workingDirectory,
  ...(execPath ? { execPath } : {}),
  logpath: path.join(workingDirectory, 'logs'),
});
...
switch (command.toLowerCase()) {
  case 'install':
    ...
    svc.on('start', () => {
      // Same as service-install.js: node-windows' 'start' event only means
      // the (detached, elevated) start command was issued, not that the
      // service actually reached RUNNING - confirm with the SCM.
  ...
  case 'status':
    ...
    try {
      const output = execSync(`sc query ${SERVICE_QUERY_NAME}`, { encoding: 'utf8' });
