...
// Create the service. NODE_ENV/PORT are deliberately NOT pinned via `env`
// here: winsw bakes those into the service definition and they would
// silently override any later edits to %ProgramData%\gmgmt\.env (dotenv
// never overrides pre-set environment variables) - the .env seeded by
// ensureServiceEnvironment is the single source of configuration.
const svc = new Service({
  ...
  workingDirectory,
  ...(execPath ? { execPath } : {}),
  // Without an explicit logpath, winsw writes its logs next to the
  // generated daemon exe inside Program Files; point it at the logs dir
  // the README documents.
  logpath: path.join(getDataDir(), 'logs'),
...
svc.on('start', () => {
  // The event only means the start command was issued - confirm with the
  // Service Control Manager before reporting success.
